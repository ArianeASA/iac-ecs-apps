...
resource "aws_ecs_service" "app-ecs-service" {
 ...
 network_configuration {
   subnets          = [for subnet in aws_subnet.app-private-subnet : subnet.id]
...
resource "aws_apigatewayv2_vpc_link" "vpclink_apigw_to_alb" {
  ...
  subnet_ids = [for subnet in aws_subnet.app-public-subnet : subnet.id]
}
...
resource "aws_nat_gateway" "app-nat-gateway" {
  count      = 2
  subnet_id     = "${element(aws_subnet.app-public-subnet.*.id, count.index)}"
...
resource "aws_route_table_association" "private" {
  count          = 2
  subnet_id      = "${element(aws_subnet.app-private-subnet.*.id, count.index)}"
